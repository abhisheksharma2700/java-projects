...
public class BusServiceImplementation implements BusService {
    ...
    public String deleteByBusId(Long id) {
        BusEntity bus = busRepository.findById(id).orElseThrow(() -> new NotFoundException("bus not found with this id" + id));
        long bookedSeats=bookingRepository.countByBus(bus);
        if (bookedSeats>0){
            throw new NotFoundException("cannot delete bus with existing bookings");
        }
   ...
   @Override
   public void validateBusRequest(BusRequestDto dto) {
        if (dto.getName() == null || dto.getBusNumber() == null ||
                dto.getSource() == null || dto.getDestination() == null ||
                dto.getDateOfDeparture() == null || dto.getTimeOfDeparture() == null) {
            throw new NotFoundException("Bus must have name, number, source, destination, date, and time of departure.");
        }
        if (dto.getTotalSeats() <= 0) {
            throw new NotFoundException("Total seats must be greater than zero.");
        }
